...
function renderSetup(){if(!setupState)return;const list=element('setupApps');const apps=setupState.apps||[];if(list){list.innerHTML=apps.length?apps.map(app=>{const state=!app.reachable?'failed':app.credentials?'healthy':'action_required';const label=!app.reachable?'Not found':app.credentials?'Ready':app.action==='temporary_password_required'?'One-time password may be required':'API key pending';return `<div class="attention-item"><div class="attention-name">${esc(app.name)}</div><div><span class="status ${state}"><span class="status-dot"></span>${label}</span><div class="detail">${esc(app.detail)}</div></div><a class="open-link" href="${esc(app.link)}" target="_blank" rel="noreferrer">Open</a></div>`;}).join(''):'<div class="empty">Run detection to find the Umbrel Arr apps already installed by the user.</div>';}setText('setupCount',setupState.detectionComplete?`${setupState.detectedCount} of ${setupState.requiredCount} apps detected`:'Detection has not run yet');setText('setupComplete',setupState.phase==='confirmed'?'Setup complete. Umbrelarr restores consent and storage selection from API-owned Prowlarr markers.':setupState.confirmed?'Consent is recorded. Finish the action below to complete setup.':'Nothing will be configured until you confirm this setup.');const confirm=element('confirmSetup');if(confirm)confirm.disabled=!setupState.canConfirm||setupState.phase==='confirmed';const panel=element('setupPanel');if(panel)panel.hidden=setupState.phase==='confirmed';if(element('reconcile'))element('reconcile').disabled=setupState.phase!=='confirmed'||Boolean(statusState?.running);renderSetupRoots();}
function renderSetupRoots(){const target=element('setupRootSelections');if(!target||!storageState)return;let selected=document.querySelector('[name="setupStorageMode"]:checked');if(!selected){const mode=storageState.actionRequired?'adopt':storageState.mode==='adopted'?'adopt':storageState.mode;selected=document.querySelector(`[name="setupStorageMode"][value="${mode}"]`);if(selected)selected.checked=true;}const adopt=selected?.value==='adopt';target.hidden=!adopt;if(adopt)target.innerHTML=storageState.libraries.map(library=>{const candidates=storageState.candidates?.[library.key]||[];const storedId=storageState.rootIds?.[library.key];const selectedId=storedId??(candidates.length===1?candidates[0].id:null);const placeholder=selectedId==null?'<option value="" disabled selected>Choose a root…</option>':'';return `<label>${esc(library.name)} ${esc(library.variant)}<select data-setup-root="${esc(library.key)}" required>${placeholder}${candidates.map(item=>`<option value="${esc(item.id)}"${String(item.id)===String(selectedId)?' selected':''}>${esc(item.path)}</option>`).join('')}</select></label>`;}).join('');const selects=[...target.querySelectorAll('select')];const rootsReady=!adopt||selects.length===storageState.libraries.length&&selects.every(select=>select.value);const confirm=element('confirmSetup');if(confirm)confirm.disabled=!setupState?.canConfirm||setupState?.phase==='confirmed'||!selected||!rootsReady;}
...
if(element('detectApps'))element('detectApps').onclick=async()=>{const button=element('detectApps');const error=element('setupError');error.textContent='';button.disabled=true;button.textContent='Detecting installed apps…';try{const response=await fetch('/api/setup/detect',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'}});const data=await response.json();if(!response.ok)throw new Error(data.error||'Detection failed');setupState=data;renderSetup();}catch(value){error.textContent=value.message;}finally{button.disabled=false;button.textContent='Detect installed apps';}};
if(element('confirmSetup'))element('confirmSetup').onclick=async()=>{const button=element('confirmSetup');const error=element('setupError');error.textContent='';button.disabled=true;button.textContent='Connecting installed apps…';const rootIds={};document.querySelectorAll('[data-setup-root]').forEach(select=>rootIds[select.dataset.setupRoot]=Number(select.value));const body=new URLSearchParams({storageMode:document.querySelector('[name="setupStorageMode"]:checked')?.value||'',rootIds:JSON.stringify(rootIds),qbittorrentUsername:element('qbittorrentUsername')?.value||'admin',qbittorrentTemporaryPassword:element('qbittorrentTemporaryPassword')?.value||''});try{const response=await fetch('/api/setup/confirm',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});const data=await response.json();if(!response.ok)throw new Error(data.error||'Setup failed');setupState=data;renderSetup();window.location='/';}catch(value){error.textContent=value.message;button.disabled=false;button.textContent='Connect detected apps';}};
...
SETUP = r"""
      <section class="panel" aria-labelledby="setupTitle">
        <div class="setup-intro"><div><h2 id="setupTitle">Connect installed Umbrel Arr apps</h2><p>Install the required apps yourself from the Umbrel Arr store. Detection is read-only and does not create containers or change app configuration. After review, confirmation creates an API-owned marker in Prowlarr and starts reconciliation.</p></div><button id="detectApps" type="button">Detect installed apps</button></div>
        ...
        <div class="panel-body" id="setupOptions">
          <fieldset><legend>Library storage</legend><label><input type="radio" name="setupStorageMode" value="local"> Umbrel storage</label> <label><input type="radio" name="setupStorageMode" value="network"> Linked network storage</label> <label><input type="radio" name="setupStorageMode" value="adopt"> Adopt existing roots</label></fieldset>
          <div id="setupRootSelections" hidden></div>
          <label for="qbittorrentUsername">qBittorrent username</label><input id="qbittorrentUsername" value="admin" autocomplete="username">
          <label for="qbittorrentTemporaryPassword">One-time qBittorrent password</label><input id="qbittorrentTemporaryPassword" type="password" autocomplete="current-password" aria-describedby="qbittorrentPasswordHelp"><small id="qbittorrentPasswordHelp">Only needed when qBittorrent reports a temporary-password action. It stays in memory and is never saved.</small>
        </div>
...
LIBRARY_PROPAGATION = r"""
      <section class="panel" aria-labelledby="propagationTitle">
        ...
        <dl class="propagation-list">
          <div class="propagation-step"><dt>Root folders</dt><dd>Creates the selected roots in both Sonarr instances, both Radarr instances, and Lidarr without removing existing roots.</dd></div>
          <div class="propagation-step"><dt>Download categories</dt><dd>Configures matching TV, movie, 4K, and music categories in qBittorrent and SABnzbd, then links both clients to every primary Arr app.</dd></div>
          <div class="propagation-step"><dt>Connected apps</dt><dd>Synchronizes the five Arr apps into Prowlarr, connects the supported video libraries to Bazarr, Profilarr, and Overseerr, and verifies each result.</dd></div>
        </dl>
      </section>
"""
